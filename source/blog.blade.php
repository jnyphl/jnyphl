    ...
    @if ($pagination->pages->count() > 1)
        <nav class="flex text-base my-8">
            ...
            @foreach ($pagination->pages as $pageNumber => $path)
                <a href="{{ $path }}" title="Go to Page {{ $pageNumber }}" class="bg-grey-lighter hover:bg-grey-light rounded mr-3 px-5 py-3 {{ $pagination->currentPage == $pageNumber ? 'text-blue-forest' : '' }}">{{ $pageNumber }}</a>
            @endforeach
